...
#include "LumeGameMode.generated.h"
 ...
 */
DECLARE_MULTICAST_DELEGATE_TwoParams(FOnLumeGameModePlayerInitialized, AGameModeBase* /*GameMode*/, AController* /*NewPlayer*/);

/**
 * @class ALumeGameMode
 * @brief The base game mode class used by this project.
 */
UCLASS(MinimalAPI, Config = Game, Meta = (ShortTooltip = "The base game mode class used by this project."))
class ALumeGameMode : public AModularGameModeBase
{
	GENERATED_BODY()
	
public:

	UE_API ALumeGameMode(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintCallable, Category = "Lume|Pawn")
	UE_API const ULumePawnData* GetPawnDataForController(const AController* InController) const;
	...
	FOnLumeGameModePlayerInitialized OnGameModePlayerInitialized;
